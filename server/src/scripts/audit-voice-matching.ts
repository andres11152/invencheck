...
async function main() {
  ...
  logger.log(`Auditando ${articulos.length} artículos del catálogo real...`);
  ...
  for (const articulo of articulos) {
    ...
    const match = await articuloService.normalizarEntradaHablada(fraseUsada);
  ...
  const resumen = {
    ...
    MATCH_INCORRECTO: casos.filter((c) => c.resultado === 'MATCH_INCORRECTO')
      .length,
    AMBIGUO_DETECTADO: casos.filter((c) => c.resultado === 'AMBIGUO_DETECTADO')
      .length,
  ...
  const outPath = resolve(__dirname, '../../audit-voice-matching-report.json');
